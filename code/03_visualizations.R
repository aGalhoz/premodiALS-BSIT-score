...
p = grouped_violin_plot(score_status_V0_V1_reduced, "ALS, CTR and PGMC scores at V0 and V1")
save_plot_pdf(p, filename = "plots/grouped_violin_ALS_CTR_PGMC.pdf",width = 8,height = 6)
p = grouped_violin_plot(score_status_V0_V1 %>% filter(visit == "V0"), "ALS, CTR and PGMC scores at V0") 
save_plot_pdf(p, filename = "plots/grouped_violin_V0_ALS_CTR_PGMC.pdf",width = 8,height = 6)

# pairwise violin plots
p = pairwise_plots(score_status_V0_V1_reduced %>% filter(status == "PGMC"),"PGMC at V0 and V1",'#ad5291')
save_plot_pdf(p, filename = "plots/pairwise_violin_PGMC.pdf")
p = pairwise_plots(score_status_V0_V1_reduced %>% filter(status == "ALS"),"ALS at V0 and V1",'#B2936F')
save_plot_pdf(p, filename = "plots/pairwise_violin_ALS.pdf")
p = pairwise_plots(score_status_V0_V1_reduced %>% filter(status == "CTR"),"CTR at V0 and V1",'#6F8EB2')
save_plot_pdf(p, filename = "plots/pairwise_violin_CTR.pdf")
